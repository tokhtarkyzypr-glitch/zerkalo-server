...
function buildPrompt({ cut, haircolor, brows, lips, makeup }) {
  ...
  if (haircolor) changes.push(`цвет волос: ${haircolor}, насыщенный, глубокий, не тусклый`);
...
Крайне важно: не меняй ракурс съёмки, угол наклона камеры, приближение, кадр и композицию
кадра. Результат должен быть буквально тем же снимком, с теми же пропорциями и границами
кадра, что и оригинал, только с указанными выше изменениями. Не разворачивай и не
перекадрируй сцену, не добавляй новые элементы фона.

Результат — фотография, а не иллюстрация. Сохрани естественную текстуру кожи с порами
и микрорельефом. Волосы с отдельными прядями и естественным блеском, цвет насыщенный
и глубокий, без пластикового глянца. Свет на новых волосах должен совпадать с освещением сцены.
...
изменение ракурса или кадрирования.`;
}
    ...
    const r = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          // соотношение сторон нарочно не задаём: если оно отличается
          // от оригинала, модель может перекадрировать сцену целиком,
          // именно это и ломало ракурс на некоторых фото
        },
      }),
    });
